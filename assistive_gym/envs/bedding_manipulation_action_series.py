...
class BeddingManipulationSeriesEnv(AssistiveEnv):
    def __init__(self, robot, human, use_mesh=False):
        if robot is None:
            super(BeddingManipulationSeriesEnv, self).__init__(robot=None, human=human, task='bedding_manipulation', obs_robot_len=12, obs_human_len=0, frame_skip=1, time_step=0.01, deformable=True, opts='2_interactions')
    ...
    def step(self, action):
        ...
        dist_reward_1, clipped_1 = self.cloth_interaction(a1, data)
        data = p.getMeshData(self.blanket, -1, flags=p.MESH_DATA_SIMULATION_MESH, physicsClientId=self.id)
        dist_reward_2, clipped_2 = self.cloth_interaction(a2, data)
        ...
        reward_distance_btw_grasp_release = dist_reward_1 + dist_reward_2
        ...
        info = {'split_reward':split_reward, 'total_point_counts':point_counts,'post_action_point_counts': post_action_point_counts, 'clipped':[clipped_1, clipped_2]}
    ...
    def cloth_interaction(self, action, data):
        ...
        return -150 if np.linalg.norm(grasp_loc - release_loc) >= 1.5 else 0, clipped
    ...
    def reset(self):
        super(BeddingManipulationSeriesEnv, self).reset()
        print('\nSERIES\n')
